...
myform = form.Form( 
    form.Textbox("Enter movie name"),
    form.Textarea("Prediction"),
    form.Button('Predict hit/flop'))

class hello:
    def GET(self):
        form = myform()
        form['Prediction'].value=""
    ...
    def POST(self): 
        ...
        if not form.validates(): 
            moviename = (form['Enter movie name'].value)
            form['Prediction'].value="error"
            return render.formtest(form)
            #return render.output(form, moviename, prediction)
        else:
            moviename = (form['Enter movie name'].value)
            X=create(moviename)
            if X==0:
                form['Prediction'].value="error: no match in IMDB"
                return render.formtest(form)
